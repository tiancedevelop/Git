...
@interface MainTxtImgCommentViewCtl ()<HYActivityViewDelegate>{
...
@implementation MainTxtImgCommentViewCtl
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tIndex == 0) {
        ...
        [self refreshCellView:cell index:0];
...
- (IBAction)zanAction:(id)sender{
    ...
    if ([[Common shareCommon] isLogin]) {
        ...
        NSArray * tZanAry = [UserDefalut objectForKey:ZanThemAry];
        if (tZanAry == nil) {
            tZanAry = [[NSMutableArray alloc] init];
        }
        NSArray * tCaiAry = [UserDefalut objectForKey:CaiThemAry];
        for (int index = 0; index < [tZanAry count]; index++) {
            NSString    * tThemId = [tZanAry objectAtIndex:index];
...
- (void)GetResult:(ASIHTTPRequest *)request
{
    ...
    if (_requestType == RequestThemCommentType) {
        ...
        if (status == 1) {
            ...
            [self performSelector:@selector(hideHud) withObject:nil afterDelay:1.0];
            NSDictionary   * tDic = [NSDictionary dictionaryWithObjectsAndKeys:_txtView.text,@"content",[UserDefalut objectForKey:UserName],@"name",[UserDefalut objectForKey:UserIconPath],@"avatar",@"0",@"rzcount",@"0",@"iszanreply", nil];
...
- (void)shareVideoToWX:(id)sender
{
    ...
    if (_scene == WXSceneTimeline) {
        message.title = [NSString stringWithFormat:@"%@:%@",AppName,[_themDic objectForKey:@"content"]];
    }else{
