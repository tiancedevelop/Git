...
- (void)mergeAndExportVideosAtFileURLs:(NSArray *)fileURLArray
{
    ...
    NSMutableArray * assetTrackAudioAry = [[NSMutableArray alloc] init];
    NSMutableArray *assetArray = [[NSMutableArray alloc] init];
    for (NSURL *fileURL in fileURLArray) {
        AVAsset *asset = [AVAsset assetWithURL:fileURL];
        CMTime assetTime = [asset duration];
        float tt = CMTimeGetSeconds(assetTime);
        if (!asset) {
            continue;
        }
...
//        AVAssetTrack *assetAudioTrack = [[asset tracksWithMediaType:AVMediaTypeAudio] objectAtIndex:0];
        NSArray * tAssetAudioTrackAry = [asset tracksWithMediaType:AVMediaTypeAudio];
        if ([tAssetAudioTrackAry count] != 0) {
            
            [assetTrackAudioAry addObject:[tAssetAudioTrackAry objectAtIndex:0]];
        }
    ...
    for (int i = 0; i < [assetArray count] && i < [assetTrackArray count] ; i++) {
        ...
        if ([assetTrackAudioAry count] != 0) {
            AVAssetTrack * assetAudioTrack = [assetTrackAudioAry objectAtIndex:i];
