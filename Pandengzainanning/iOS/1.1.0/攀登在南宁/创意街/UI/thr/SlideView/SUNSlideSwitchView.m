...
- (void)createNameButtons
{
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        //累计每个tab文字的长度
        topScrollViewContentWidth += 80;
        xOffset = (80-textSize.width)/2+80*i;
        //设置按钮尺寸
        ...
        [button setTag:i+100];
        if (i == 0) {
            _shadowImageView.frame = CGRectMake((IphoneWidth/4-textSize.width)/2, 0, textSize.width, kHeightOfTopScrollView);
            button.selected = YES;
        }
