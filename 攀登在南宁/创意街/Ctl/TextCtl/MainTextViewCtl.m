...
@interface MainTextViewCtl ()<WXApiDelegate,HYActivityViewDelegate>
{
    enum WXScene        _scene;
    ...
    MainTxtCommentViewCtl   * _mainTxtCommentViewCtl;
...
@implementation MainTextViewCtl
...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(shareToWxResult:) name:ShareToWxResultNotification object:nil];
...
- (void)getThem:(NSString *)sTime
{
    ...
    NSMutableDictionary     * tDic = [[NSMutableDictionary alloc] initWithObjectsAndKeys:@"2",@"type_id",sTime,@"stime", [UserDefalut objectForKey:UserUid],@"uid",nil];
...
{
    if (nil == _mainTxtCommentViewCtl) {
        _mainTxtCommentViewCtl = [[MainTxtCommentViewCtl alloc] initWithNibName:@"MainTxtCommentView" bundle:nil];
    }
    [_mainTxtCommentViewCtl setThemDic:[_themMutableAry objectAtIndex:index]];
    [self.navigationController pushViewController:_mainTxtCommentViewCtl animated:YES];
    [self.tabBarController.tabBar setHidden:YES];
}
