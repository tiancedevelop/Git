...
- (void)requestStarted
{
	...
	if (nil != delegate && [delegate respondsToSelector:didStartSelector]) {
...
+ (void)registerForNetworkReachabilityNotifications
{
	...
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reachabilityChanged:) name:kReachabilityChangedNotificationAsi object:nil];
}


+ (void)unsubscribeFromNetworkReachabilityNotifications
{
	[[NSNotificationCenter defaultCenter] removeObserver:self name:kReachabilityChangedNotificationAsi object:nil];
}
