...
@property (strong, nonatomic) UIButton *localFileButton;
...
- (void)initPreview
{
    self.preview = [[UIView alloc] initWithFrame:CGRectMake(0, 44, DEVICE_SIZE.width, DEVICE_SIZE.width)];
...
- (void)initProgressBar
{
    ...
    [SBCaptureToolKit setView:_progressBar toOriginY:DEVICE_SIZE.width+44];
...
- (void)initLocalBtn
{
    CGFloat okButtonW = 100;
    self.localFileButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, okButtonW, okButtonW)];
//    _localFileButton.enabled = NO;
    
    [_localFileButton setTitle:@"本地视频..." forState:UIControlStateNormal];
    [SBCaptureToolKit setView:_localFileButton toOrigin:CGPointMake((self.view.frame.size.width - okButtonW)/2, self.view.frame.size.height  - 30)];
    
    [_localFileButton addTarget:self action:@selector(selectLocalVideo) forControlEvents:UIControlEventTouchUpInside];
    
    CGPoint center = _localFileButton.center;
    center.y = _recordButton.center.y+_recordButton.frame.size.height+10;
    _localFileButton.center = center;
    
    [self.view insertSubview:_localFileButton belowSubview:_maskView];
}
...
- (void)pressCloseButton
{
    if ([_recorder getVideoCount] > 0) {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"提醒" message:@"真的要放弃这个视频?" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"放弃", nil];
...
//        long long fileLength = [self fileSizeAtPath:[outputFileURL ]];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    if ([[info objectForKey:UIImagePickerControllerMediaType] isEqualToString:(NSString*)kUTTypeMovie]) {
        ...
        NSLog(@"%@",videoPath);
        PlayViewController *playCon = [[PlayViewController alloc] initWithNibName:@"PlayViewController" bundle:nil withVideoFileURL:[NSURL URLWithString:videoPath]];
        [self.navigationController pushViewController:playCon animated:YES];
